...
data <- readChar("BioPortalApiKey.txt", nchars = 36) #This gets the apikey from a txt file
...
getRecommendedTerms <- function(dataSet) {  # Get a list of terms to standardize
  ...
  rURL <<- sprintf("http://data.bioontology.org/recommender?input={%s&apikey=%s&display_links=false&display_context=false",rowChar,data)
...
server <- function(input, output, session) { 
      ...
      tryCatch({
        res <- R.utils::withTimeout(  { 
          ...
          if (diffNum > DAYS_SINCE_DOWNLOAD){ 
            tryCatch({
              res <- R.utils::withTimeout({
                myContent <- RJSONIO::fromJSON(paste0("http://data.bioontology.org/ontologies?apikey=",data))
  ...
  observeEvent(input$automatch, ignoreInit = T, {
    ...
    withProgress(message = 'Automatching Data', value = 0, {
      ...
      if (is.null(input$editThisColumn) || input$editThisColumn == "") {
        ...
        sURL <<- sprintf("http://data.bioontology.org/annotator?text=%s&apikey=%s",gsub(" ", "", URLencode(uniqueTerms, reserved = TRUE)),data)
  ...
  observe({
    if (!is.null(input$editColumn) && nchar(input$editColumn) > 0) {
      ...
      aURL <<- sprintf("http://data.bioontology.org/annotator?text=%s&apikey=%s",input$editColumn,data)
